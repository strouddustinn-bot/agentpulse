...
class CheckinRequest(BaseModel):
    ...
    status: Literal["ok", "attention", "error"]
    observations: NonNegativeInt = 0
    breaches: NonNegativeInt = 0
    actions: NonNegativeInt = 0
    queued: NonNegativeInt = 0
    alerts: NonNegativeInt = 0
    anomalies: NonNegativeInt = 0
    escalations: NonNegativeInt = 0
    ...
    async def checkin(
        ...
        try:
            recorded = app_store.record_checkin(
                org_id=principal.org_id, payload=dict(payload)
            )
