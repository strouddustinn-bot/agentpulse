...
Optional backend check-ins for the hosted/control-plane API.
...
|---|---|---|
| `endpoint_url` | URL string | AgentPulse backend check-in endpoint. Leave empty to disable backend check-ins. |
| `auth_token` | string | Bearer token generated by `agentpulse-backend create-api-key`. Sent as `Authorization: Bearer ...`. |
| `timeout_seconds` | number | Network timeout for check-in POSTs. Backend failures are reported but never stop local monitoring/remediation. |

When configured, `agentpulse run` and `agentpulse run-once` send a best-effort check-in after each non-dry-run cycle. `--dry-run` never posts to the backend.
